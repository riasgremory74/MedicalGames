...
public class StartBoost2500 : MonoBehaviour {
    ...
    public void StartBoost()
    {
        ...
        PlayerPrefs.SetFloat("BoostDistance2500", 2500f);
    }
    private void FixedUpdate()
    {
        if (BoostActive == true)
        {
            CountdownText.text = (PlayerPrefs.GetFloat("BoostDistance2500") - (Player.transform.position.z - FirstPosition)).ToString("0");
        }
        if (Player.transform.position.z < 0)
        {

        }
        else if (PlayerPrefs.GetFloat("StartBoost2500Counter") >= 1 && startBoost == true)
        {
            PlayerPrefs.SetFloat("StartBoost2500Counter", PlayerPrefs.GetFloat("StartBoost2500Counter") - 1);
        ...
        if ((Player.transform.position.z - FirstPosition) > PlayerPrefs.GetFloat("BoostDistance2500") && active == true)
        {
            Flash.SetActive(false);
            if (delay > 0)
            {
